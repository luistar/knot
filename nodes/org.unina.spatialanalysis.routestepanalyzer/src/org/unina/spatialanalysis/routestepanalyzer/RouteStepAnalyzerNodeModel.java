...
public class RouteStepAnalyzerNodeModel extends NodeModel {
	 ...
	 * {@link RouteStepAnalyzerNodeDialog}. The settings model will sync via the
	 * above defined key.
	 * 
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec)
		...
		while (rowIterator.hasNext()) {
			...
			} else if (ownerId == Integer.MIN_VALUE || theGeom == null || beginAt == null || endAt == null
					|| originId == Long.MIN_VALUE || destinationId == Long.MIN_VALUE) {
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
		...
		if (colIDName == null || colBeginAtName == null || colEndAtName == null || colOriginIDName == null
				|| colDestIDName == null || colTagsName == null || colGeometryName == null) {
		...
		if (!(inSpecs[0].containsName(colIDName) && inSpecs[0].containsName(colGeometryName)
				&& inSpecs[0].containsName(colBeginAtName) && inSpecs[0].containsName(colEndAtName)
				&& inSpecs[0].containsName(colOriginIDName) && inSpecs[0].containsName(colDestIDName)
			...
			if (!(inSpecs[0].getColumnSpec(colIDName).getType().getCellClass().equals(IntCell.TYPE)
				...
				throw new InvalidSettingsException(
						"The input columns must have the following type: owner_id-Integer, begin_at-DateTime, end_at-DateTime, origin_id-Long, destination_id-Long, origin_tags-String, destination_tags-String, the_geom-String");
			}
			throw new InvalidSettingsException(
					"The input table must contain the following columns: owner_id, begin_at, end_at, origin_id, destination_id, origin_tags, destination_tags, the_geom");
		}
	...
	private DataTableSpec createOutputForDetailed() {
		...
		newColumnSpecs.add(specCreator.createSpec());
		specCreator = new DataColumnSpecCreator("MedianTime(" + TimeSlot.EARLY_MORNING.toString() + ")",
				DoubleCell.TYPE);
		newColumnSpecs.add(specCreator.createSpec());
	...
	private DataTableSpec createOutputForSimple() {
		...
		specCreator.setProperties(properties);
		simpleColumnSpecs.add(specCreator.createSpec());
		DataColumnSpec[] simpleColumnSpecsArray = simpleColumnSpecs
				.toArray(new DataColumnSpec[simpleColumnSpecs.size()]);
