...
public class RouteCalculatorNodeModel extends NodeModel {
	...
	private static final String OWNER_ID_COL = "m_col_owner_id";

	private static final String ROUTE_ID_COL = "m_col_route_id";

	private static final String TIMESTAMP_ID_COL = "m_col_timestamp";
	...
	private final SettingsModelColumnName m_colOwnerIDSettings = createColOwnerIDSettings();

	private final SettingsModelColumnName m_colRouteIDSettings = createColRouteIDSettings();
	...
	static SettingsModelColumnName createColOwnerIDSettings() {
		SettingsModelColumnName ownerIDColSettingModel = new SettingsModelColumnName(OWNER_ID_COL, null);
		ownerIDColSettingModel.setEnabled(true);
		return ownerIDColSettingModel;
	}
	
	static SettingsModelColumnName createColRouteIDSettings() {
		SettingsModelColumnName routeIDColSettingModel = new SettingsModelColumnName(ROUTE_ID_COL, null);
		routeIDColSettingModel.setEnabled(true);
		return routeIDColSettingModel;
	}
	
	static SettingsModelColumnName createColTimestampSettings() {
		SettingsModelColumnName rowLocationSettingModel = new SettingsModelColumnName(TIMESTAMP_ID_COL, null);
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec)
		 ...
		 */
		String colOwnerIDName = m_colOwnerIDSettings.getColumnName();
		String colRouteIDName = m_colRouteIDSettings.getColumnName();
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
		 ...
		 */
		if(!(inSpecs[0].containsName(colRouteIDName) && inSpecs[0].containsName(colTimestampName) && inSpecs[0].containsName(colGeometryName)) && inSpecs[0].containsName(colOwnerIDName)) {
			if(!(inSpecs[0].getColumnSpec(colRouteIDName).getType().getCellClass().equals(LongCell.TYPE) &&
					inSpecs[0].getColumnSpec(colTimestampName).getType().getCellClass().equals(DateAndTimeCell.TYPE) && 
							inSpecs[0].getColumnSpec(colGeometryName).getType().getCellClass().equals(StringCell.TYPE)) &&
					inSpecs[0].getColumnSpec(colOwnerIDName).getType().getCellClass().equals(IntCell.TYPE)) {
	...
	protected void saveSettingsTo(final NodeSettingsWO settings) {
		 ...
		 */
		m_colOwnerIDSettings.saveSettingsTo(settings);
		m_colRouteIDSettings.saveSettingsTo(settings);
	...
	protected void loadValidatedSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
		 ...
		 */
		m_colOwnerIDSettings.loadSettingsFrom(settings);
		m_colRouteIDSettings.loadSettingsFrom(settings);
	...
	protected void validateSettings(final NodeSettingsRO settings) throws InvalidSettingsException {
		 ...
		 * variables.
		 */
		m_colOwnerIDSettings.validateSettings(settings);
		m_colRouteIDSettings.validateSettings(settings);
