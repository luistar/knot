...
public class TrajectoryAssignerNodeModel extends NodeModel {
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec) throws Exception {
		...
		for(int i = 0; i<specs.getNumColumns(); i++) {
			...
			if(columnspec.getName().equals("id")) {
				idIndex = i;
			}else if(columnspec.getName().equals("timestamp")) {
				timeStampIndex = i;
			}else if(columnspec.getName().equals("location")) {
				locationIndex=i;
			}
		}
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
		...
		if(!(inSpecs[0].containsName("id") && inSpecs[0].containsName("timestamp") && inSpecs[0].containsName("location"))) {
			if(!(inSpecs[0].getColumnSpec("id").getType().getCellClass().equals(IntCell.TYPE) &&
					inSpecs[0].getColumnSpec("timestamp").getType().getCellClass().equals(StringCell.TYPE) && 
							inSpecs[0].getColumnSpec("location").getType().getCellClass().equals(StringCell.TYPE))) {
